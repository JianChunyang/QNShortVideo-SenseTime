...
extern "C" {
    ...
    JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_processTextureBoth(JNIEnv * env, jobject obj, jint textureIn, jobject humanAction, jint rotate, jint frontRotate, jint imageWidth, jint imageHeight, jboolean needsMirroring, jobject inputParams, jobjectArray animalFace, jint animalFaceCount, jint textureOut);
    ...
    JNIEXPORT jlong JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_getAnimalDetectConfig(JNIEnv * env, jobject obj);
...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_createInstanceNative(JNIEnv * env, jobject obj)
{
    ...
    st_mobile_sticker_set_param_ptr(sticker_handle, -1, ST_STICKER_PARAM_PACKAGE_STATE_FUNC_PTR, (void*)&packageEvent);
    st_mobile_sticker_set_param_ptr(sticker_handle, -1, ST_STICKER_PARAM_MODULE_ANIMATION_EVENT_CALLBACK_PTR, (void*)&animationEvent);
    st_mobile_sticker_set_param_ptr(sticker_handle, -1, ST_STICKER_PARAM_MODULE_KEY_FRAME_EVENT_CALLBACK_PTR, (void*)&keyFrameEvent);
...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_processTextureBoth(JNIEnv * env, jobject obj, jint textureIn, jobject humanAction, jint rotate, jint frontRotate, jint imageWidth, jint imageHeight, jboolean needsMirroring, jobject inputParams, jobjectArray animalFace, jint animalFaceCount, jint textureOut){
    ...
    st_mobile_animal_face_t *animal_face = new st_mobile_animal_face_t[animalFaceCount];
    for(int i = 0; i < animalFaceCount; i++){
        if(!convert2AnimalFace(env, env->GetObjectArrayElement(animalFace, i), &animal_face[i])){
            memset(&animal_face[i], 0, sizeof(st_mobile_animal_face_t));
        }
    }
    ...
    if (stickerhandle != NULL) {
        //result  = st_mobile_sticker_process_texture(stickerhandle, textureIn, imageWidth, imageHeight,  (st_rotate_type)rotate,needsMirroring, &human_action, item_callback, textureOut);
        result = st_mobile_sticker_process_texture_both(stickerhandle,
                                                              textureIn, imageWidth, imageHeight, (st_rotate_type) rotate, (st_rotate_type) frontRotate, needsMirroring,
                                                              &human_action, &input_params, animal_face, animalFaceCount, textureOut);
        LOGI("-->>st_mobile_sticker_process_and_output_texture --- result is %d", result);
    }

    releaseHumanAction(&human_action);
    releaseAnimal(animal_face, animalFaceCount);
...
JNIEXPORT jlong JNICALL Java_com_sensetime_stmobile_STMobileStickerNative_getAnimalDetectConfig(JNIEnv * env, jobject obj)
{
    ...
    {
        unsigned long long action = -1;
        int result = st_mobile_sticker_get_animal_detect_config(stickerhandle, &action);
        if (result == ST_OK) {
